...
MIL_CODE_DLLFUNC void   MFTYPE McodeWriteA     (MIL_ID ModelCodeId, MIL_ID ImageBufId, MIL_CONST_TEXTA_PTR StringPtr, MIL_INT64  ControlFlag);
...
MIL_CODE_DLLFUNC void   MFTYPE McodeWriteW     (MIL_ID ModelCodeId, MIL_ID ImageBufId, MIL_CONST_TEXTW_PTR StringPtr, MIL_INT64  ControlFlag);
...
inline void   MFTYPE McodeGetResultSafeType(MIL_ID ResultCodeId, MIL_INT64  ResultType, std::nullptr_t              );
#endif
...
inline MIL_INT MFTYPE McodeInquireSafeTypeExecute (MIL_ID ContextOrResultCodeId, MIL_INT64  InquireType, void  *UserVarPtr, MIL_INT64  GivenType)
   ...
   if((RequiredType != M_TYPE_DOUBLE) && (RequiredType != M_TYPE_MIL_INT32) && (RequiredType != M_TYPE_MIL_INT64) &&
      (RequiredType != M_TYPE_MIL_ID) && (RequiredType != M_TYPE_CHAR) && (RequiredType != M_TYPE_TEXT_CHAR) && (RequiredType != M_TYPE_STRING))
      RequiredType = 0;
   if(RequiredType == 0)
      {
...
inline void MFTYPE McodeGetResult(MIL_ID ResultCodeId, MIL_INT64  ResultType, std::vector<UserType> &UserVarArrayPtr)
   ...
   MIL_INT InternalNumberOfElementsForStdVector = 0;
   McodeGetResult(ResultCodeId, M_STRIP_HLVLDATATYPE(ResultType) + M_NB_ELEMENTS + M_TYPE_MIL_INT, &InternalNumberOfElementsForStdVector);

   UserVarArrayPtr.resize(InternalNumberOfElementsForStdVector);

   if(InternalNumberOfElementsForStdVector > 0)
      {
      McodeGetResult(ResultCodeId, ResultType, &UserVarArrayPtr[0]);
      }
   }
template <typename UserType>
...
inline void MFTYPE McodeWriteW(MIL_ID ModelCodeId, MIL_ID ImageBufId, const MIL_STRING& StringPtr, MIL_INT64  ControlFlag)
   {
   return McodeWriteW(ModelCodeId, ImageBufId, StringPtr.c_str(), ControlFlag);
   }
...
inline void MFTYPE McodeWriteA(MIL_ID ModelCodeId, MIL_ID ImageBufId, const MIL_STRING& StringPtr, MIL_INT64  ControlFlag)
   {
   return McodeWriteA(ModelCodeId, ImageBufId, StringPtr.c_str(), ControlFlag);
   }
...
inline void MFTYPE McodeWrite(MIL_ID ModelCodeId, MIL_ID ImageBufId, const MIL_STRING& StringPtr, MIL_INT64  ControlFlag)
   {
   return McodeWrite(ModelCodeId, ImageBufId, StringPtr.c_str(), ControlFlag);
   }
...
inline void   MFTYPE McodeGetResultSafeType(MIL_ID ResultCodeId, MIL_INT64  ResultType, MIL_STRING     &UserVarArrayPtr)
   ...
   McodeGetResultSafeType(ResultCodeId, (ResultType & (~M_HLVLDATATYPE_MASK)) + M_STRING_SIZE + M_TYPE_MIL_INT, &InternalStringSize);

   if(InternalStringSize > 0)
      {
      UserVarArrayPtr.assign(InternalStringSize, MIL_TEXT('\0'));
      McodeGetResultSafeTypeExecute(ResultCodeId, ResultType, &UserVarArrayPtr[0], M_TYPE_TEXT_CHAR);
...
inline void MFTYPE McodeGetResult(MIL_ID ResultCodeId, MIL_INT64  ResultType, MIL_STRING &UserVarArrayPtr)
   ...
   MIL_INT InternalStringSize = 0;
   McodeGetResult(ResultCodeId, (ResultType & (~M_HLVLDATATYPE_MASK)) + M_STRING_SIZE + M_TYPE_MIL_INT, &InternalStringSize);
