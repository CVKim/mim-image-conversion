...
#define M_QT_MODE                                     M_USE_DISPLAY_SIZE_BUFFER
#define M_GTK_MODE                                    M_USE_DISPLAY_SIZE_BUFFER
...
inline void MdispControl(MIL_ID DisplayId,
   ...
   {
   MdispControl(DisplayId, ControlType, M_PTR_TO_DOUBLE(ControlValue));
   };
...
inline void MdispControl(MIL_ID DisplayId, MIL_INT64 ControlType, const MIL_STRING& ControlValue)
   ...
   if(M_IN_DISP_INQUIRE_STRING_RANGE(ControlType))
      {
      MdispControl(DisplayId, ControlType, M_PTR_TO_DOUBLE(ControlValue.c_str()));
      }
   else
      {
#if M_MIL_USE_SAFE_TYPE
...
inline MIL_INT MFTYPE MdispInquireSafeType(MIL_ID DispId, MIL_INT64 InquireType, MIL_STRING      &UserVarPtr)
   ...
   //Test if InquireType in String range
   if(M_IN_DISP_INQUIRE_STRING_RANGE(InquireType))
      {
      MIL_INT InternalStringSize = 0;
      MdispInquireSafeType(DispId, InquireType + M_STRING_SIZE, &InternalStringSize);

      if(InternalStringSize > 0)
         {
         UserVarPtr.assign(InternalStringSize, MIL_TEXT('\0'));
         RetValue = MdispInquireExecute(DispId, InquireType, &UserVarPtr[0], M_TYPE_TEXT_CHAR);
         UserVarPtr.resize(InternalStringSize - 1);
         }
      else
         {
         SafeTypeError(MIL_TEXT("MdispInquire: Failed to inquire string size"));
         }
      }
   else
      {
      //Generate SafeTypeError and force exit
      SafeTypeError(MIL_TEXT("MdispInquire: Inquire type not in string range"));
      }
   return RetValue;
   }
#endif /* M_MIL_UNICODE */
